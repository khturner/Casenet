...
getCharges <- function(case_number) {
  # Get search results
  ...
  # Return final df
  data.frame(case_number = case_number, case_style = case_style,
...
shinyServer(function(input, output) {
    ...
    if (length(case_numbers) > 0) {
      withProgress({
        for (i in 1:length(case_numbers)) {
          ...
          result <- rbind(result, getCharges(case_number)) %>% tbl_df
        }
  ...
  output$results <- renderDataTable({ charges() })
  
  output$download <- downloadHandler("cases.csv", function(filename) {
    write_csv(charges(), filename)
  })
})
